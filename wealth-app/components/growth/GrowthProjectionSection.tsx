...
export function GrowthProjectionSection() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-white">Growth Projection</h2>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="border-[--border] bg-[--card] lg:col-span-1">
          <CardHeader className="pb-2">
            <CardTitle className="text-base text-white">Your inputs</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label className="text-slate-300">
                Monthly savings:{" "}
                <span className="font-bold text-violet-400">${monthly}</span>
              </Label>
              <Slider min={50} max={2000} step={50} value={[monthly]} onValueChange={([v]) => setMonthly(v)} />
              <div className="flex justify-between text-xs text-slate-500"><span>$50</span><span>$2,000</span></div>
            </div>

            <div className="space-y-2">
              <Label className="text-slate-300">
                Current savings:{" "}
                <span className="font-bold text-cyan-400">${currentSavings.toLocaleString()}</span>
              </Label>
              <Slider min={0} max={50000} step={500} value={[currentSavings]} onValueChange={([v]) => setCurrentSavings(v)} />
              <div className="flex justify-between text-xs text-slate-500"><span>$0</span><span>$50K</span></div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-xs text-slate-500"><span>1 yr</span><span>40 yrs</span></div>
            </div>

            <div className="space-y-2">
              <Label className="text-slate-300">Account / return rate</Label>
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Final value", value: formatCAD(finalValue), color: "text-violet-400" },
              { label: "You contribute", value: formatCAD(totalContributed), color: "text-cyan-400" },
              { label: "Market growth", value: formatCAD(growth), color: "text-emerald-400" },
            ].map((s) => (
            ...
            <CardContent className="pt-4">
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={data} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  />
                  {milestonesHit.map((m) => (
                    <ReferenceLine key={m} y={m} stroke="#2d3148" strokeDasharray="4 4"
                      label={{ value: formatCAD(m), position: "insideTopRight", fill: "#475569", fontSize: 10 }} />
                  ))}
                  <Line type="monotone" dataKey="withSavings" stroke="#7c3aed" strokeWidth={2.5} dot={false} />
                  <Line type="monotone" dataKey="contributionsOnly" stroke="#22d3ee" strokeWidth={1.5} strokeDasharray="5 5" dot={false} />
                  <Line type="monotone" dataKey="withoutSavings" stroke="#475569" strokeWidth={1} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
